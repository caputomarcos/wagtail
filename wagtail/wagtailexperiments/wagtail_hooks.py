...
from wagtail.wagtailexperiments import admin_urls
...
class ExperimentButtonHelper(ButtonHelper):
    def report_button(self, pk, classnames_add=[], classnames_exclude=[]):
        ...
        return {
            'url': reverse('wagtailexperiments:report', args=(quote(pk), )),
            'label': _('Show report'),
            'classname': cn,
...
def check_wagtailexperiments(page, request, serve_args, serve_kwargs):
    # If the page being served is the goal page of an experiment, log a completion
    completed_wagtailexperiments = Experiment.objects.filter(goal=page, status='live')

    if completed_wagtailexperiments:
        user_id = get_user_id(request)

        for experiment in completed_wagtailexperiments:
    ...
    wagtailexperiments = Experiment.objects.filter(control_page=page, status__in=('live', 'completed'))
    if wagtailexperiments:
        experiment = wagtailexperiments[0]
